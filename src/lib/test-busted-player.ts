...
async function runTest() {
    ...
    log("Initial State: " + store.getState().players.map(p => `${p.name}: ${p.stack}`).join(", "));
    ...
    try {
        ...
        log("Hand 1 Started. Active Players: " + hand.playerHands.map(ph => ph.playerId).join(", "));
    ...
    log("\n--- Starting Hand 2 ---");
    try {
        const { hand: hand2, updatedPlayers: players2 } = initializeHand(store.getState());
        store.setState({ currentHand: hand2, players: players2 });

        log("Hand 2 Started. Active Players: " + hand2.playerHands.map(ph => ph.playerId).join(", "));
