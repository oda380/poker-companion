...
export function InitialDealDialog() {
  ...
  const needsConfirm =
    currentHand.activePlayerId === "WAITING_FOR_DEAL_CONFIRM";

  if (needsConfirm) {
    console.log(
      "InitialDealDialog: Rendering because activePlayerId is WAITING_FOR_DEAL_CONFIRM"
    );
  }
      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription className="text-base">
            {isHoldem && (
              <>
                <strong>2 hole cards</strong> have been dealt face-down to each
                player.
              </>
            )}
            {isStud && (
              <>
                <strong>1 hole card</strong> (face-down) and{" "}
                <strong>1 door card</strong> (face-up) have been dealt to each
                player.
              </>
            )}
          </DialogDescription>
        ...
        <div className="space-y-4">
          <div className="bg-muted/50 rounded-lg p-4">
            ...
            <div className="flex flex-wrap gap-2">
              {activePlayers.map((player) => (
                <div
                  key={player.id}
                  className="px-3 py-1 bg-background rounded-full text-sm font-medium border"
                >
                  {player.name}
          ...
          <Button
            size="lg"
            className="w-full h-14 text-lg bg-gradient-to-br from-emerald-600 to-emerald-700 hover:from-emerald-700 hover:to-emerald-800 text-white"
