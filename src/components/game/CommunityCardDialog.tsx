...
function CommunityCardForm() {
  ...
  const handleCardSelect = (cardCode: string) => {
    ...
    // Ensure array is big enough (though initialized correctly, safe to keep)
    ...
    if (filledCards.length === cardsNeeded) {
      ...
      usePokerStore.setState((state) => {
        ...
        if (activePlayers.length < 2) {
          ...
          const upcomingStreet = getNextStreetName(
            state.currentHand.currentStreet
          );
          ...
          const nextActive =
            sortedActive.find((p) => p.seat > state.currentHand!.dealerSeat) ||
            sortedActive[0];
      ...
      // No need to reset selectedCards as the component will unmount/remount or parent will hide it
    }
  };

  const isComplete =
    selectedCards.filter((c) => c !== "").length === cardsNeeded;
    ...
    <>
      ...
      <div className="space-y-6">
        <div className="text-center space-y-4">
          <div
            className={cn(
              "inline-flex items-center justify-center px-4 py-1.5 rounded-full font-bold tracking-wider text-sm border",
